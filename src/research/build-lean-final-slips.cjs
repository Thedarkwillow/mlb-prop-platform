...
function mergeEnrichment(row, indexes) {
  ...
  return {
    ...
    resolvedTeam: row.resolvedTeam ?? enriched.resolvedTeam ?? row.team ?? enriched.team ?? null,
    resolvedGame: row.resolvedGame ?? enriched.resolvedGame ?? row.game ?? enriched.game ?? null,
    resolvedGamePk: row.resolvedGamePk ?? enriched.resolvedGamePk ?? row.gamePk ?? enriched.gamePk ?? null,
    homeAway: row.homeAway ?? enriched.homeAway ?? null,
    pitcherHand: row.pitcherHand ?? enriched.pitcherHand ?? row.opposingPitcherHand ?? enriched.opposingPitcherHand ?? null,
    opposingPitcher: row.opposingPitcher ?? enriched.opposingPitcher ?? row.probablePitcher ?? enriched.probablePitcher ?? null,
...
function normalizeCandidate(row, source) {
  ...
  return {
    ...
    resolvedGamePk: row.resolvedGamePk ?? row.gamePk ?? null,
