  ...
  return rows
    .flatMap(r => {
      ...
      return sides.map(side => ({
        ...
        game: r.game ?? r.matchup ?? r.resolvedGame ?? null,
        gamePk: r.gamePk ?? r.resolvedGamePk ?? null,
        market,
        side,
        line,
        prob,
        edge: Number(r.adjustedEdge ?? r.edge ?? r.sportsbookAdjustedEdge ?? r.sportsbookEdge ?? 0),
        books,
...
async function main() {
  const rawBoard = getPrizePicksRows();
  const rows = rawBoard;
  ...
  fs.writeFileSync(`outputs/history/${DATE}-prizepicks-board-graded.json`, JSON.stringify(supported, null, 2));
  fs.writeFileSync(`outputs/history/${DATE}-prizepicks-board-unmatched.json`, JSON.stringify(unsupported, null, 2));

  const histPath = "data/results/prizepicks-board-history.json";
  ...
  const learning = {
    ...
    note: "PrizePicks full-board grading: all gradeable PrizePicks rows; learning-only, not direct ROI.",
  ...
  fs.writeFileSync("data/learning/prizepicks-board-market-learning.json", JSON.stringify(learning, null, 2));

  console.log(`PRIZEPICKS BOARD GRADING ${DATE}`);
  console.log("Raw PrizePicks rows:", rawBoard.length);
  console.log("PrizePicks prop rows:", rows.length);
  ...
  console.log("Wrote:");
  console.log(`outputs/history/${DATE}-prizepicks-board-graded.json`);
  console.log(`outputs/history/${DATE}-prizepicks-board-unmatched.json`);
  console.log("data/results/prizepicks-board-history.json");
  console.log("data/learning/prizepicks-board-market-learning.json");
}
