...
function compactRow(layer, row, indexes) {
  ...
  return {
    ...
    classification: row.classification || row.candidateClass || row.class || null,
    confidence: row.confidence || row.validationConfidence || null,
    confidenceScore: firstNumber(row.confidenceScore, row.validationConfidenceScore),
    stakeGuidance: row.stakeGuidance || null,
    note: row.note || row.reasonNote || null,
    reasons: Array.isArray(row.reasons) ? row.reasons : [],
    score: firstNumber(row.score, row.finalScore),
    volatility: row.volatility || null,
    officialEligible: row.officialEligible ?? null,
