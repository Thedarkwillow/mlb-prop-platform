...
const OUT_FILE = "data/learning/demon-market-trust.json";
...
function actionFor({ market, side, sample, hitRate }) {
  if (side !== "MORE") {
    return {
      action: "SUPPRESS",
      reason: "demon_non_more_not_allowed"
    };
  }
  ...
  if (market === "strikeouts") {
    return {
      action: "SUPPRESS",
      reason: "raw_demon_strikeouts_more_extremely_bad"
    };
  }
  ...
  if (hitRate >= 0.45) {
    return {
      action: "ALLOW",
      reason: "raw_demon_hit_rate_60_plus"
    };
  }
  ...
  if (hitRate >= 0.35) {
    return {
      action: "WATCH",
      reason: "raw_demon_hit_rate_watch"
    };
  }

  return {
    action: "SUPPRESS",
    reason: "raw_demon_hit_rate_below_threshold"
  };
}
...
function listReports() {
  ...
  return fs.readdirSync(HISTORY_DIR)
    .filter(f => /^\d{4}-\d{2}-\d{2}-demon-raw-performance\.json$/.test(f))
...
for (const file of files) {
  ...
  for (const r of report.rows) {
    ...
    if (String(r.oddsTier || r.tier || "").toLowerCase() !== "demon") continue;
...
const output = {
  ...
  source: "outputs/history/*-demon-raw-performance.json",
  reports: files.length,
  gradedRows: rows.length,
  policy: {
    suppressBelowHitRate: 0.35,
    watchBelowHitRate: 0.45,
    minSampleForAllow: 30,
    hardSuppress: [
      {
        market: "strikeouts",
        side: "MORE",
        reason: "raw demon strikeouts MORE has been extremely poor"
      }
    ]
  },
  markets
...
console.log("DEMON MARKET TRUST");
console.log("===================");
console.log({
  reports: files.length,
  gradedRows: rows.length,
